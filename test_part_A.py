...
param_data = pytest.mark.parametrize(
    "data",
    [
...
def test_loop(data):
    ...
    actual = np.std(data)
    assert np.isclose(actual, expected), "{} is not close to {}".format(
        expected, actual
    )
    assert round(actual, 7) == round(
        expected, 7
    ), "Rounded {} is not equalt to {}".format(expected, actual)
...
param_edge_data = pytest.mark.parametrize(
    "edge_cases",
    [
        [],
        [1, 2, "a", 3],
